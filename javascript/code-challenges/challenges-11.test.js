...
function transformToLis(obj) {
  ...
  for (let i = 0; i < objEntries.length; i++) {
    newArr.push(`<li>${objEntries[i][0]}: ${objEntries[i][1]}</li>`);
  }
  return newArr;
}
...
const count = (target, input) => {
  ...
  function arrayCheckAndCount(countNumber, arrayInput) {
    arrayInput.map((value) => {
      if (Array.isArray(value)) {
        arrayCheckAndCount(countNumber, value);
      } else {
        if (value === target) {
          targetCount++;
        }
  ...
  if (Array.isArray(input)) {
    arrayCheckAndCount(target, input);
  }
  return targetCount;
};
...
const totalSum = (input) => {
  ...
  function arrayCheckAndCount(arrayInput) {
    arrayInput.map((value) => {
      if (Array.isArray(value)) {
        arrayCheckAndCount(value);
      } else {
...
let findShortest = (data) => {
  let shortest;
  let shortestHeight = 9999;
  data.forEach(ele => {
    console.log(`ele.height(${ele.height}) < shortestHeight(${shortestHeight}) = `,ele.height < shortestHeight);
