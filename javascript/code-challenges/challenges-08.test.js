...
const sortByChildren = (charArray) => {
  let funArr = charArray;
  return funArr.sort((a, b) => {
    if (
...
const citiesAtoJ = (arr) => {
  let regexThing = /^[A-J]\w+/g;
  let filt = arr.filter(val => {
    return val.match(regexThing);
  })
  // console.log('>>>>>>>>>>>>>>>>>>>',filt);
  return filt;
};
